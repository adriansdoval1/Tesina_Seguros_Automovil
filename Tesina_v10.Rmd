...
Para poder entender el modelo de XGradient Boosting (por sus siglas en inglés Extreme Gradient Boosting) es necesario dar una breve introducción a los Árboles de Regresión que son la base de muchas metodologías de Machine Learning.

## 3.1 Árbol de Regresión

Un Árbol de Regresión es un modelo estadístico de aprendizaje automático supervisado que se entrena con un conjunto de datos en el que cada observación cuenta con variables explicativas (entrada) y una variable objetivo conocida (salida). Su propósito es predecir un valor numérico continuo. Para lograrlo, el modelo divide iterativamente los datos en subconjuntos cada vez más homogéneos, utilizando criterios que minimizan una función objetivo, como el error cuadrático medio observado. 
...
El Árbol de Regresión mostrado en el ejemplo contiene 2 nodos internos y 3 nodos terminales. Cada nodo terminal indica la predicción de Millas por Galón de Gasolina, la cantidad de observaciones que contiene el nodo, así como el porcentaje que estas representan del total. Por ejemplo, el primer nodo terminal (de izquierda a derecha) predice que las Millas por Galón de Gasolina para los vehículos que tienen un peso mayor o igual a 2.4 toneladas y una potencia (caballos de fuerza) superior a 178 son 14. Además, este nodo agrupa 10 observaciones, lo que representa aproximadamente el 31% del conjunto de datos.
...
En la gráfica se aprecia que para construir un Árbol de Regresión se definieron 3 regiones que no están superpuestas. Estas regiones corresponden a los 3 nodos terminales del gráfico del Árbol de Regresión. Así mismo, para obtener una predicción de las Millas por galón de Gasolina se calcula el promedio de todas las observaciones de cada región.

De manera general para poder obtener estimaciones de un Árbol de Regresión se siguen los siguientes pasos:
...
Del ejemplo anterior, obtuvimos 3 regiones ($R_1$, $R_2$, y $R_3$) y el valor promedio de la variable respuesta en cada región es 14, 20 y 29 Millas por Galón de Gasolina respectivamente. Aunque podría existir otras divisiones el algoritmo de $rpart$ encontró que estas regiones minimizan la función objetivo.

Para poder construir las regiones $R_1$,...,$R_j$ no superpuestas que minimicen una función objetivo, es necesario definir tanto una forma factible de dividir el espacio de las variables explicativas como la función objetivo a optimizar. En este caso, la función objetivo utilizada es el Error Cuadrático (EC) que se define de la siguiente manera: 
...
Esta función objetivo es muy usada enla literatura y se encuentra implementada en la mayoría de las paqueterías de $R$. Además de ser una función convexa y doblemente diferenciable. Sin embargo, se pueden minimizar otras como el Error Porcentual mientras sean funciones diferenciables de segundo orden y convexa ya que esto garantiza que tiene un mínimo.

Dado que considerar todas las posibles particiones del espacio de variables explicativas resulta computacionalmente inviable, se recurre a un proceso llamado particionamiento recursivo binario descendente. Es decir, se comienza con un único nodo que contiene todas las observaciones y, en cada paso, se divide el espacio de las variables explicativas en dos ramas utilizando una de las variables explicativas. En cada iteración, se selecciona la división que minimiza la función objetivo, optimizando localmente el ajuste del modelo.

Entonces, primero se selecciona la variable predictora $X_i$ y se selecciona un punto de corte $s$ tal que las regiones $R_1=\{X_i|X_i<s\}$ y $R_2=\{X_i|X_i>=s\}$ minimicen el EC. Después se selecciona otra variable $X_i$ y se hace lo mismo. La variable que se selecciona para hacer la primera partición es aquella que minimice el EC. De esta manera se consideran todas las posibles variables explicativas.

Aunque el procedimiento descrito anteriormente garantiza un árbol que minimiza el EC en los datos de entrenamiento, existe el riesgo de que el Árbol de Regresión resultante sea demasiado grande y tenga muchos nodos terminales (sea muy complejo) por lo que termine estando sobreajustado. Es decir, puede ajustarse demasiado a las particularidades del conjunto de entrenamiento y, por lo tanto, generar predicciones poco confiables en nuevos datos. Para evitar este sobreajuste y, al mismo tiempo, no generar un árbol excesivamente simple (con muy pocos nodos), se incorpora un término de penalización de complejidad a la función objetivo a minimizar.
...
Mientras $\alpha$ se haga grande, vamos a tener un Árbol de Regresión mucho menos complejo, pero si $\alpha=0$ regresamos al caso anterior es decir al Árbol de Regresión sin restricciones. El propósito es tener diferentes tipos de Árboles de Decisión en función de $\alpha$ y seleccionar aquel que haga que el Árbol de Regresión no sea lo suficientemente simple ni complejo.

El problema del Árbol de Regresión se reduce a encontrar las regiones minimicen la función objetivo anterior con un término de penalización de complejidad, yes la base de muchos problemas de Machine Learning.

Finalmente, la selección de $\alpha$ la hace la paquetería de $rpart$ de $R$ usando validación cruzada (Cross Validation) que es una técnica estadística utilizada para evaluar el desempeño de un modelo predictivo de manera más robusta. Consiste en dividir el conjunto de datos en $K$ subconjuntos del mismo tamaño. El procedimiento se repite $K$ veces, de modo que en cada iteración se utiliza uno de los subconjuntos como conjunto de validación, y los $K-1$ restantes como conjunto de entrenamiento.


## 3.2 XGRadient Boosting

El Modelo XGradient Boosting (por sus siglas en inglés Extreme Gradient Boosting) es una extensión optimizada de las metodologías de Gradient Boosting Machines y AdaBoost. Se encuentra implementada a través de la paquetería $xgboost$ con la función $xgboost$ en $R$. De manera general la metodología de XGradient Boosting consiste en entrenar secuencialmente Árboles de Regresión pequeños usando los residuos como variable objetivo para los siguientes árboles hasta obtener un nivel adecuado de precisión.

El algoritmo para estimar el modelo a un grupo de datos de tamaño $m$, con $n$ variables explicativas y una variable obejtivo $y$ es el siguiente:
...
Usualmente se usan Árboles de Regresión de poca profundidad y se estiman usando un criterio que minimice una función objetivo. De acuerdo a Gareth James et al. la mejor técnica para encontrar el hiperparámetro $b$ es usando Cross Validation, así mismo el mismo autor sugiere que el hiperparámetro $\eta$ varía de acuerdo al problema.

Aunque no hay una forma definitiva para encontrar el parámetro $p$, de acuerdo a Gareth James et al. un parámetro de $p=1$ funciona bien. Sin embargo, para este trabajo se decide encontrar usando validación cruzada. 

El siguiente diagrama muestra de manera visual como el algoritmo genera diferentes Árboles de Regresión usando recursivamente los residuos mientras se minimiza alguna función objetivo. Al final una predicción es la suma ponderada del resultado de cada árbol de decisión. 

```{r, echo = FALSE, message = FALSE, fig.align = 'center', fig.cap = "Diagrama de 3 Iteraciones del Algoritmo del Modelo de XGradient Boosting", out.width="60%" }
...
De acuerdo a Tianqi Chen y Carlos Guestrin (2016) se usa la siguiente función objetivo para encontrar el Árbol de Regresión en el paso $t$ ($t<=b$).
...
Por lo que se encontró una forma de representar la t-ésima estimación en una suma de las t estimaciones anteriores más el estimado del Árbol de Regresión en t. Sustituyendo en la ecuación original, se puede expresar la función objetivo en t como:
...
$$
Se puede aproximar la función $l(.)$ usando una expansión de segundo orden de Taylor alrededor de $\hat{y}_i^{(t-1)}$:
...
$$
En consecuencia, en la iteración $t$, la función objetivo a minimizar se reduce a la siguiente expresión, excluyendo los términos constantes que no influyen en la optimización.

$$
\text{obj}^{(t)} \approx \sum_{i=1}^n \left[ g_i f_t(x_i) + \frac{1}{2} h_i f_t(x_i)^2 \right] + \omega(f_t)
$$

Ahora definiremos el término $\omega(f_t)$ que cumple la función de penalizar la complejidad del Árbol de Regresión de manera similar a como se explicó en el capítulo anterior. Primero definamos la estructura de un Árbol de Regresión como $f_t(x)=w_{q(x)}$ con $w \in \mathbb{R}^T$ y $q:\mathbb{R}^n \rightarrow \{1,2,...,T\}$ donde $T$ es el número de nodos terminales en el t-ésimo Árbol de Regresión. La función $f_t(x)$ es un vector con los valores estimados en cada nodo terminal del Árbol de Regresión en $t$, y $q$ es una función que asigna cada dato a un nodo terminal.

Tianqi Chen y Carlos Guestrin (2016) usan la siguiente definición:
...
$$
Así terminamos con la siguiente expresión para la función objetivo del t-ésimo Árbolde Regresión.

$$
\text{obj}^{(t)} = \sum_{j=1}^{T} \left[ G_j w_j + \frac{1}{2} (H_j + \lambda) w_j^2 \right] + \gamma T
$$
De esta manera podemos usar cualquier función a minimizar como el EC. Así mismo esta ecuación se puede minimizar derivando con respecto a $w_j$ usando técnicas analiticas y obtenemos el valor óptimo para $w_j$.

$$
w_j^{*}=-\frac{G_j}{H_j + \lambda}
$$
Entonces, nos dice que las predicciones que minimizan la función objetivo es la anterior. Así, mismo substituyendo en la función objetivo observamos que el mínimo es:

$$
\text{obj}^{(*)}  = -\frac{1}{2}\sum_{j=1}^{T} \frac{G_j^2}{H_j + \lambda} + \gamma T
$$

La expresión anterior se puede usar como una forma para evaluar la estructura de un Árbol de Regresión y es llamado el "Score".

El "Score" sirve como medida para poder evaluar que tan importante es una variable al usarla en un Árbol de Regresión durante la estimación. La paquetería de $xgboost$ tiene la función $xgb.plot.importance$ que calcula el "Gain" de todas las variables. El "Gain" mide cuánto mejora el modelo (reduce el error) cuando se utiliza una variable para hacer una división en un árbol.
...
La función de $xgb.plot.importance$ se encarga de calcular el Gain por cada variable de cada árbol y al final presenta cual variable fué la que redujo más el error. 

Aunque una de las principales ventajas del modelo XGradient Boosting es que suele ofrecer predicciones más precisas que un Árbol de Regresión individual, esta mejora en el desempeño viene acompañada de una pérdida en la interpretabilidad del modelo. 
...
Por lo que procedemos a hacer el análisis de importancia de variables con el "Score".
...
En el ejemplo se observa que la variable más importante fué el peso del automóvil ("wt") ya que representa más del 50% del Gain total del modelo.
...
Esta variable se identifica en la base como "Elegibilidad_UBER". Clasifica el auto cotizado cumple los criterios para prestar servicio de transpote privado en la aplicación de UBER en 2021. De acuerdo a la página de UBER, las características mínimas para que el automóvil pueda ser usado son:
...
Como se observa, los vehículos que no cumplen todas las características para poderse usar en la aplicación de UBER tienen una distribución con una media inferior a aquellos vehículos que se pueden usar. Además es visible la existencia de una relación positiva entre la elegibildad de UBER con el precio del seguro. Esta relación tiene sentido ya que los requisitos mínimos para poder usar el vehículo en la aplicación de transporte privado UBER reune característica del vehículo que lo pueden hacer más valioso por lo que incrementa el precio del seguro por lo que se usará en el modelo. Es importante notar que existe la posibilidad de colinealidad con otras variables que se usarán en los modelos; sin embargo, se usó por que se desean conservar todas las características del automóvil, y se hará un análisis de significancia en los modelos para identificar la permanencia de la variable.
...
Para el modelo lineal se usó la función $xgboost$ con la paquetería de $xgboost$ de R. Para éste modelo el valor de los hiperparámetros es crucial. Aunque el modelo tiene muchos hiperparámetros se decidió enfocarse en los siguientes que se consideran los principales:

+ $nrounds$, número máximo de iteraciones del algoritmo. Limitado a ser mayor o igual a $1$.
+ $eta$, la tasa de aprendizaje del modelo que está entre $0$ y $1$. La documentación del paquete indica que mientras el valor sea más cercano a 0 se tiene un modelo con menos sobre ajuste. Además, entre más bajo sea también se debe aumentar el número de iteraciones. Su valor por defecto es $0.3$.
...
```{r, echo=FALSE, message = FALSE, results = "hide", fig.align = "center", fig.height = 2, fig.width = 5, fig.cap = "Importancia de las Variables"}
...
Se observa que existe una relación lineal poco dispersa entre los Valores Estimados con los Valores Reales.
